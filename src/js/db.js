...
const initDB = function () {
    const /** {JSON | undefined} */ db = localStorage.getItem("notebookDB");

    if (db) {
        notebookDB = JSON.parse(db);
    } else {
        notebookDB.notebooks = [];
        localStorage.setItem("notebookDB", JSON.stringify(notebookDB));
    }
};
...
/**
 * Reads and loads the localStorage data in to the global variable 'notebookDB'.
 */
const readDB = function () {
    notebookDB = JSON.parse(localStorage.getItem("notebookDB"));
};

/**
 * Writes the current state of the global variable 'notebookDB' to local storage
 */
...
export const db = {
    post: {
        ...
        notebook(name) {
            ...
            notebookDB.notebooks.push(notebookData);
    ...
    update: {
        ...
        note(noteId, object) {
            ...
            const /** {Object} */ oldNote = findNote(notebookDB, noteId);
    ...
    delete: {
        ...
        notebook(notebookId) {
            ...
            notebookDB.notebooks.splice(notebookIndex, 1);
        ...
        note(notebookId, noteId) {
            ...
            const /** {Object} */ noteIndex = findNoteIndex(notebook, noteId);
